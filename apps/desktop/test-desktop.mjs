...
async function launch() {
  if (packaged) {
    ...
    return _electron.launch({ executablePath: exe });
  }
...
rmSync(vaultFile, { force: true }); // clean slate from any earlier run
...
await app.close();
rmSync(vaultFile, { force: true }); // don't leave a test vault on the machine
